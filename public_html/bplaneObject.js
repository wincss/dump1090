...
var symbolCenter = new BMap.Size(32, 32);

var trackLine = new BMap.Polyline([], {
    strokeColor: '#000000',
	strokeOpacity: 1.0,
	strokeWeight: 3,
...
var planeObject = {
	...
	funcAddToTrack	: function(){
			...
			this.trackpath.push(new BMap.Point(this.longitude, this.latitude));
		},
	...
	funcUpdateIcon	: function() {
            ...
            if (this.symbol) {
                this.symbol.setRotation(this.track);
                this.symbol.setStrokeWeight(this.is_selected ? 2 : 1);
                this.symbol.setFillColor(this.markerColor);
            } else {
                this.symbol = new BMap.Symbol("M 0,0 "+ baseSvg["planeData"], {
                    strokeWeight: (this.is_selected ? 2 : 1),
                    scale: 0.4,
                    fillColor: this.markerColor,
                    fillOpacity: 0.9,
                    anchor: symbolCenter, // Set anchor to middle of plane.
                    rotation: this.track
                });
            }
            return this.symbol;
	...
	funcUpdateMarker: function() {
			if (this.marker) {
				this.marker.setPosition(new BMap.Point(this.longitude, this.latitude));
                this.marker.setIcon(this.funcUpdateIcon());
			} else {
				this.marker = new BMap.Marker(new BMap.Point(this.longitude, this.latitude));
                this.marker.setIcon(this.funcUpdateIcon());
				this.marker.icao = this.icao;
                map.addOverlay(this.marker);

				// Trap clicks for this marker.
				this.marker.addEventListener('click', this.funcSelectPlane);
			}
	...
	funcUpdateLines: function(addOverlay) {
        trackLine.setPath(this.trackpath);
        if(addOverlay) {
